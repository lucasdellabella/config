...
if [[ -z $update ]]; then
    ...
    if [[ -z $remote ]]; then
        sudo apt install silversearcher-ag fzf nodejs neovim -y
    fi
    echo "$(tput setaf 3)RUN \`:call coc\#util\#install()\` TO SETUP VIM"; tput sgr0
